...
async fn handle_socket(socket: WebSocket, state: AppState) {
    ...
    let sender_task = tokio::spawn(async move {
        while let Ok(Some(msg)) = message_rx.recv().await {
    ...
    let mut ping_tx = message_tx.clone();
    let ping_task = tokio::spawn(async move {
        ...
        loop {
            ...
            if ping_tx.send(Some(Message::Ping(vec![]))).await.is_err() {
                break;
            }
        }
    });

    let mut forward_tx = message_tx.clone();
    let forward_task = tokio::spawn(async move {
        while let Ok(msg) = rx.recv().await {
            if forward_tx.send(Some(msg)).await.is_err() {
                break;
            }
        }
    });
